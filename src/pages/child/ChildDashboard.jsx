...
import { getChildConfig, getChildState, saveChildState, getChildren } from '../../lib/firestore'
...
export function ChildDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      // Find this child's record - try multiple strategies
      let childId = profile?.childId

      if (!childId) {
        // Strategy: find in children collection by userId
        const kids = await getChildren(adminId)
        const myRecord = kids.find(k => k.userId === user.uid)
        if (myRecord) {
          childId = myRecord.id
          setChildInfo(myRecord)
        } else {
          // Fallback: use uid as childId (created automatically by getChildren)
          childId = user.uid
        }
      }
      ...
      // Try to get child info from children collection
      if (!childInfo) {
        const kids = await getChildren(adminId)
        const myRecord = kids.find(k => k.id === childId || k.userId === user.uid)
        setChildInfo(myRecord || { childName: profile?.childName, gender: profile?.gender || 'girl', ageGroup: profile?.ageGroup })
      }
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
      <Spinner size={36} />
      <p style={{ color: 'var(--gray-400)', fontSize: '0.875rem' }}>Se încarcă...</p>
    </div>
  )
  ...
  if (error) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16, padding: 24 }}>
      <StarLogo size={56} />
      <p style={{ color: '#E24B4A', fontWeight: 600, textAlign: 'center' }}>{error}</p>
      <button onClick={handleLogout} className="btn btn-ghost btn-sm">Ieși din cont</button>
    </div>
  )
  ...
  const gender = childInfo?.gender || profile?.gender || 'girl'
  const childName = childInfo?.childName || profile?.childName || 'Tu'
        ...
        {tab === 0 && (
          <div className="fade-in">
            <div style={{ background: 'var(--amber-50)', border: '1px solid var(--amber-400)', borderRadius: 'var(--radius-md)', padding: '8px 12px', marginBottom: 12, fontSize: '0.8125rem', color: 'var(--amber-600)', display: 'flex', alignItems: 'center', gap: 6 }}>
              <span>👀</span> Sarcinile sunt bifate de părinte.
            </div>
                  ...
                  <div key={r.id} style={{ background: 'var(--white)', border: `1px solid ${!canAfford || blocked ? 'var(--gray-200)' : 'var(--purple-100)'}`, borderRadius: 'var(--radius-lg)', padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12, opacity: blocked ? 0.55 : 1 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: '0.8125rem', color: 'var(--gray-400)', marginTop: 2 }}>
                        {r.pts} ⭐{r.dailyLimit ? ` · ${dailyUsed}/${r.dailyLimit} azi` : ''}{r.weeklyLimit ? ` · ${weeklyUsed}/${r.weeklyLimit} săpt` : ''}
                      </div>
                    </div>
                    <button onClick={() => redeemReward(r)} disabled={!canAfford || blocked} className="btn btn-primary btn-sm" style={{ flexShrink: 0, opacity: !canAfford || blocked ? 0.4 : 1 }}>
